...
| `neeshops/ranking/` | Heuristic reranker (working), LLM reranker (stub) | **P3** | `list[Candidate]`, catalog lookup, state | `list[Recommendation]` | Yes | See `neeshops/ranking/README.md` |
| `neeshops/personalization/` | Soft profile → ranking boost | **P3** | catalog row, `UserProfile` | float boost in [0,1] | Yes | Single function, `personalization_boost()` — extend signature only with default-valued kwargs |
